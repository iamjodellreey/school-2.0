<x-main-app-layout>
    ...
    <x-form :route="route('setups.fee.category.amount.update', $editData[0]->fee_category_id)">
            ...
            <x-dropdown name="fee_category_id" label="fee_category_id" selectName="Fee" selectId="amountId">
                ...
                @foreach ($fee_categories as $category)
                    <option value="{{ $category->id }}"
                        {{ selected(intval(old('fee_category_id', $editData['0']->fee_category_id)), $category->id) }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </x-dropdown>
                ...
                <div class="delete_whole_extra_item_add" id="delete_whole_extra_item_add">
                    <x-dropdown name="class_id[]" label="class_id[]" selectName="Student Class" selectId="classId">
                        ...
                        @foreach ($classes as $class)
                            <option value="{{ $class->id }}"
                                {{ selected(intval(old('class_id[]', $edit->class_id)), $class->id) }}>
                                {{ $class->name }}
                            </option>
                        @endforeach
                    </x-dropdown>
                    <x-input class="{{ is_filled($errors->all(), $edit) }}">
                        ...
                        <input name="amount[]" type="text" class="form-control"
                            value="{{ old('amount[]', $edit->amount) }}" required>
                    </x-input>
                </div>
            @endforeach
