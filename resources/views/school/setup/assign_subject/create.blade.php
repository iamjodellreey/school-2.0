<x-main-app-layout>
    ...
    <x-form :route="route('setups.assign.subject.store')">

        @if (!empty($errors->all()))
            <div class="row mb-3">
                <x-input-error :messages="$errors->get('full_mark[]')" />
                <x-input-error :messages="$errors->get('pass_mark[]')" />
                <x-input-error :messages="$errors->get('subjective_mark[]')" />
            </div>
        @endif
        <div class="add_item">
            ...
            <x-input class="{{ is_filled($errors->all()) }}">
                ...
                <input name="full_mark[]" type="text" class="form-control" value="{{ old('full_mark[]') }}" required>
            </x-input>
            <x-input class="{{ is_filled($errors->all()) }}">
                <x-input-label>
                    <x-slot:label>
                ...
                <input name="pass_mark[]" type="text" class="form-control" value="{{ old('pass_mark[]') }}" required>
            </x-input>
            <x-input class="{{ is_filled($errors->all()) }}">
                <x-input-label>
                    <x-slot:label>
                ...
                <input name="subjective_mark[]" type="text" class="form-control"
                    value="{{ old('subjective_mark[]') }}" required>
            </x-input>
